...
# using "include PositionMover".
# NB: Plug-ins such as "acts_as_list" offer additional functionality and better
# performance.
module PositionMover

  # <tt>move_to_position</tt> is an instance method that will move a list item to
  # a new position, but also increment/decrement the positions of the other list
  # items as necessary.
  #
  ...
  # <tt>position_scope</tt> defines an SQL fragment used for narrowing the scope
  # of queries related to position.
  #
  # Often it is not desirable to manage positions for all items in a class, but
  # instead to narrow the scope based on a parent class. For example, if subject1
  # has 3 pages and subject2 has 4 pages, you would want to narrow the scope of
  # position so that when working with subject1's pages only positions 1-3 under
  # that subject are reordered. The positions of subject2's pages should be
  # unchanged. And each subject should be able to have a page at position 1.
  #
  # To narrow the scope, override this method in your model with the SQL that 
